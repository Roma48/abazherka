	...
	public function getTitle($url, $case)
	{
		...
		switch ($case)
		{
			case 'lowercase':
				...
				break;
			case 'uppercase':
				$title = utf8_strtoupper($title);
				break;
			case 'uppercasefirst':
				$title = utf8_strtoupper(utf8_substr($title, 0, 1))
					. utf8_strtolower(utf8_substr($title, 1));
				break;
			case 'titlecase':
				$title = function_exists('mb_convert_case')
					? mb_convert_case(utf8_strtolower($title), MB_CASE_TITLE)
			...
			case 'titlecase_smart':
				$title           = function_exists('mb_convert_case')
					? mb_convert_case(utf8_strtolower($title), MB_CASE_TITLE)
					: ucwords(strtolower($title));
				$lowercase_words = explode(',', ' ' . str_replace(',', ' , ', utf8_strtolower($this->params->lowercase_words)) . ' ');
