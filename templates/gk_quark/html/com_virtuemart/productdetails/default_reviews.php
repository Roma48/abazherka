...
<form method="post"
     ...
     <div class="rating">
          <label for="vote"> <span title="Rating" class="vmicon ratingbox" style="display:inline-block;width:55px;"></span> </label>
     ...
     <?php
				...
				$reviewJavascript = "
		jQuery(function($) {
			var box = jQuery('.rating .ratingbox');
			jQuery('.rating .ratingbox').mousemove( function(e){
				var parentPos = box.offset().left;
				var rating = Math.floor((e.pageX - parentPos) / 10);
				box.css('background-position', '0 ' + ((5 - rating) * -10) + 'px');
				$('#vote').val(rating);
			});
		});
		";
...
<?php
		...
		echo '<div class="customer-greetings">';
...
<div class="list-reviews">
     ...
     <div class="<?php echo $color ?>">
          <div>
               <h3><?php echo $review->customer ?></h3>
               <span class="date"><?php echo JHtml::date ($review->created_on, vmText::_ ('DATE_FORMAT_LC')); ?></span> <span class="vote"><span class="vmicon" style="background-position: 0 <?php echo ((5 - (int)$review->review_rating) * -10) . 'px'; ?>;" title="Rating: <?php echo (int)$review->review_rating; ?>/5"></span></span></div>
          <div>
               <p>
