<fieldset class="vm-fieldset-pricelist">
	...
	width="100%">
          <tr>
               ...
               <th style="min-width:70px;width:5%;align:right;text-align:center"><?php echo vmText::_ ('COM_VIRTUEMART_CART_PRICE') ?></th>
               <th style="min-width:120px;width:10%;align:right;text-align:center"><?php echo vmText::_ ('COM_VIRTUEMART_CART_QUANTITY') ?> / <?php echo vmText::_ ('COM_VIRTUEMART_CART_ACTION') ?></th>
		...
		?>
               <th style="min-width:76px;width:5%;align:right;text-align:center">
                    <?php  echo "<span  class='priceColor2'>" . $tax . '</span>' ?>
               </th>
               <?php } ?>
               ...
               <td align="right">
				...
				?>
				   ...
				   title="<?php echo  vmText::_('COM_VIRTUEMART_CART_UPDATE') ?>" class="quantity-input js-recalculate" size="3" maxlength="4" name="quantity[<?php echo $pkey; ?>]" value="<?php echo $prow->quantity ?>" />
                    <input type="submit" class="vmicon vm2-add_quantity_cart" name="updatecart.<?php echo $pkey ?>" title="<?php echo  vmText::_ ('COM_VIRTUEMART_CART_UPDATE') ?>" value="" />
                    <input type="submit" class="vmicon vm2-remove_from_cart" name="delete.<?php echo $pkey ?>" title="<?php echo vmText::_ ('COM_VIRTUEMART_CART_DELETE') ?>" value="" />
               </td>
               <?php if (VmConfig::get ('show_tax')) { ?>
...
if (VmConfig::get ('coupons_enable')) {
	?>
          <tr class="sectiontableentry2 coupon">
...
foreach ($this->cart->cartData['DATaxRulesBill'] as $rule) {
	...
	(!VmConfig::get('oncheckout_opc',true) and VmConfig::get('oncheckout_show_steps',false) and
		!empty($this->cart->virtuemart_shipmentmethod_id) )
          ...
          <tr class="sectiontableentry1 tablespace" style="vertical-align:top;">
               ...
               <td colspan="4" style="align:left;vertical-align:top;"> <small><?php echo strip_tags(str_replace('<span class="vmshipment_description', ' - <span class="vmshipment_description', $this->cart->cartData['shipmentName'])); ?></small><br/>
                    <?php
	...
	?>
               <td colspan="4" style="align:left;vertical-align:top;"> <?php echo $this->cart->cartData['shipmentName']; ?> </td>
               <?php } ?>
               <?php if (VmConfig::get ('show_tax')) { ?>
	...
	( 	VmConfig::get('oncheckout_opc',true) or
		...
		( (!VmConfig::get('oncheckout_opc',true) and VmConfig::get('oncheckout_show_steps',false) ) and !empty($this->cart->virtuemart_paymentmethod_id))
          ...
          <tr class="sectiontableentry1 tablespace" style="vertical-align:top;">
               ...
               <td colspan="4" align="left"> <small><?php echo strip_tags(str_replace('<span class="vmpayment_description', ' - <span class="vmpayment_description', $this->cart->cartData['paymentName'])); ?></small><br/>
                    <?php
		...
		echo vmText::_ ('COM_VIRTUEMART_CART_PAYMENT');
               ...
               <?php } else { ?>
               <td colspan="4" style="align:left;vertical-align:top;"><?php echo $this->cart->cartData['paymentName']; ?> </td>
               <?php } ?>
               <?php if (VmConfig::get ('show_tax')) { ?>
